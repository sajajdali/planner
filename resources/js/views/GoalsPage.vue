...
const goalPlaceholders = computed(() => {
    const placeholders = {
        numeric: {
            title: 'مثلاً: پس‌اندازم را به ۱۰۰ میلیون تومان برسانم',
            why: 'مثلاً: می‌خواهم برای خرید لپ‌تاپ و خیال راحت‌تر پولم را منظم جمع کنم.',
            start: '۲۰',
            target: '۱۰۰',
            unit: 'میلیون تومان',
        },
        doable: {
            title: 'مثلاً: نسخه اول سایت شخصی‌ام را منتشر کنم',
            why: 'مثلاً: می‌خواهم نمونه‌کارهایم یک جای مرتب و قابل ارسال داشته باشد.',
            completion: 'مثلاً: دامنه وصل باشد و صفحه‌ها بدون خطا باز شوند',
            output: 'مثلاً: یک سایت آنلاین با صفحه معرفی، نمونه‌کار و تماس',
        },
        habit: {
            title: 'مثلاً: هفته‌ای سه بار پیاده‌روی کنم',
            why: 'مثلاً: می‌خواهم انرژی روزانه‌ام بیشتر شود و بدنم از بی‌تحرکی دربیاید.',
            repeat: '۳',
        },
        milestone: {
            title: 'مثلاً: دوره طراحی رابط کاربری را کامل کنم',
            why: 'مثلاً: می‌خواهم مهارتم را مرحله‌به‌مرحله بسازم و آخرش نمونه‌کار داشته باشم.',
            stage: 'مثلاً: فصل اول را ببینم و تمرینش را انجام دهم',
        },
        ongoing: {
            title: 'مثلاً: هر شب ۲۰ دقیقه مطالعه داشته باشم',
            why: 'مثلاً: می‌خواهم مطالعه بخشی از ریتم زندگی‌ام شود، نه یک کار مقطعی.',
            minAmount: 'مثلاً: ۲۰ دقیقه',
        },
    } as const;

    return placeholders[goalDraft.value.type as keyof typeof placeholders] ?? placeholders.numeric;
});
        ...
        <div v-if="goalModal" class="goal-modal-backdrop">
            <form class="goal-modal" @submit.prevent="nextStep">
                ...
                <div v-else-if="step === 2" class="goal-form-grid">
                    <label class="full"><span>عنوان هدف</span><input v-model="goalDraft.title" required autofocus :placeholder="goalPlaceholders.title" /></label>
                ...
                <div v-else-if="step === 3" class="goal-form-grid">
                    <label class="full"><span>چرا این هدف برای من مهم است؟</span><textarea v-model="goalDraft.why" :placeholder="goalPlaceholders.why"></textarea></label>
                    <div class="success-title full">معیار موفقیت ({{ currentType?.label || '' }})</div>
                    <template v-if="goalDraft.type === 'numeric'">
                        <label><span>مقدار شروع</span><input v-model="goalDraft.start_value" inputmode="decimal" :placeholder="goalPlaceholders.start" /></label>
                        <label><span>مقدار هدف</span><input v-model="goalDraft.target_value" inputmode="decimal" :placeholder="goalPlaceholders.target" /></label>
                        <label><span>واحد</span><input v-model="goalDraft.unit" :placeholder="goalPlaceholders.unit" /></label>
                    ...
                    <template v-else-if="goalDraft.type === 'doable'">
                        <label class="full"><span>معیار تکمیل</span><input v-model="goalDraft.completion_criteria" :placeholder="goalPlaceholders.completion" /></label>
                        <label class="full"><span>خروجی مورد انتظار</span><input v-model="goalDraft.expected_output" :placeholder="goalPlaceholders.output" /></label>
                    </template>
                    <template v-else-if="goalDraft.type === 'habit'">
                        <label><span>تعداد تکرار</span><input v-model="goalDraft.repeat_count" :placeholder="goalPlaceholders.repeat" /></label>
                        ...
                        <label class="full">
                            ...
                            <div class="goal-time-field">
                                <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9"></circle><path d="M12 7v5l3 2"></path></svg>
                                <input v-model="goalDraft.suggested_time" type="time" />
                            </div>
                        </label>
                    </template>
                    <template v-else-if="goalDraft.type === 'milestone'">
                        ...
                        <div class="stage-inline full">
                            <input v-model="goalDraft.new_stage" :placeholder="goalPlaceholders.stage" @keydown.enter.prevent="addStage" />
                            <button type="button" @click="addStage">＋</button>
                        </div>
                    </template>
                    <template v-else>
                        <label class="full"><span>حداقل مقدار انجام در هر بار</span><input v-model="goalDraft.min_amount" :placeholder="goalPlaceholders.minAmount" /></label>
                    </template>
                </div>
...
@media(max-width:720px){.goal-time-field{min-height:52px;border:2px solid #eadfbe;border-radius:14px;padding:0 12px;box-shadow:2px 2px 0 rgba(58,46,31,.12)}.goal-time-field svg{width:26px;height:26px;stroke:#3a2e1f}.goal-time-field input{height:50px!important;min-height:50px!important;font-size:16px}.goal-time-field input::-webkit-calendar-picker-indicator{width:34px;height:34px}}
</style>
